...
public class Drone : MonoBehaviour {
	...
	public float SPEED = 2f;   // vitesse du drone
	public float MAX_TILT = 50f;    // Inclinaison max du drone
	...
	public void ApplyForces(float moveHorizontal, float moveVertical, float moveUp, float moveSpin)
	{
		...
		float mov_y = moveUp - (rb.velocity.y * 100f) + (rb.mass * Mathf.Abs(Physics.gravity.y));
		if (mov_y < 0) mov_y = 0;

		//Vitesse de rotation (local) du drone
		Vector3 localAngularVelocity = rb.transform.InverseTransformDirection(rb.angularVelocity);

		//Vitesse de deplacement désiré sur		GAUCHE / DROITE / AVANT / ARRIERE		.
		float desiredRight = moveHorizontal + ((orientation.z + localAngularVelocity.z * STABILITY) / MAX_TILT); // Calcul de l'angle	gauche	selon l'orientation et la vitesse de rotation du drone
		float desiredLeft = -moveHorizontal - ((orientation.z + localAngularVelocity.z * STABILITY) / MAX_TILT); // Calcul de l'angle	droit	selon l'orientation et la vitesse de rotation du drone
		float desiredRear = -moveVertical + ((orientation.x + localAngularVelocity.x * STABILITY) / MAX_TILT); // Calcul de l'angle	avant	selon l'orientation et la vitesse de rotation du drone 
		float desiredFront = moveVertical - ((orientation.x + localAngularVelocity.x * STABILITY) / MAX_TILT); // Calcul de l'angle	arriere	selon l'orientation et la vitesse de rotation du drone

		//Vitesse de rotation du drone sur lui-meme sur l'axe Y
		float desiredSpin = moveSpin - localAngularVelocity.y;

		//Calcul de la force a appliquer au drone par la		gauche / droite / avant / arriere
